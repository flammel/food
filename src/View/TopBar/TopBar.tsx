...
interface Props {
    children: Array<React.ReactElement|string|null>,
}
...
export default function TopBar(props: Props): React.ReactElement {
    ...
    return (
        <NavDrawerContext.Provider value={{isVisible: drawerVisible, toggle: () => {setDrawerVisible((prev) => !prev)}}}>
            <div className="top-bar">
                {props.children.map((child, idx) => {
                    if (typeof child === "string") {
                        return <span className="top-bar__title" key={idx}>{child}</span>
                    } else if (child !== null) {
...
const NavItem = withRouter((props: NavItemProps): React.ReactElement => {
...
export function MenuButton(): React.ReactElement {
    ...
    return (
        <button className="top-bar__nav-button" onClick={ctx.toggle}>
            <svg className="top-bar__nav-icon" viewBox="0 0 12 16" version="1.1" aria-hidden="true"><path fillRule="evenodd" d="M11.41 9H.59C0 9 0 8.59 0 8c0-.59 0-1 .59-1H11.4c.59 0 .59.41.59 1 0 .59 0 1-.59 1h.01zm0-4H.59C0 5 0 4.59 0 4c0-.59 0-1 .59-1H11.4c.59 0 .59.41.59 1 0 .59 0 1-.59 1h.01zM.59 11H11.4c.59 0 .59.41.59 1 0 .59 0 1-.59 1H.59C0 13 0 12.59 0 12c0-.59 0-1 .59-1z"></path></svg>
        </button>
    );
}
